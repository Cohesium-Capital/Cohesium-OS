...
export type MspOption = { id: string; name: string };

const NO_TARGET = "__none__";

type CsvRow = Record<string, string>;

// CSV columns we recognize (Mode C: a Google Sheets export). Converted to the
// same JSON payload the model produces.
...
export function ImportForm({
  ...
  async function run() {
    ...
    try {
      ...
      const result = await importSourced({
        rawText,
        kind,
        targetMspId: kind === "customer" && targetMsp !== NO_TARGET ? targetMsp : null,
      });
      setReport(result);
      if (result.ok) {
        toast.success(
